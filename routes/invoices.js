...
import EntryItem from "../models/entry-item.js";
...
router.get("/foruser/:userID", async (req, res) => {
  ...
  try {
    ...
    if (!invoiceList || invoiceList.length === 0) {
      return res
        .status(404)
        .json({ success: false, message: "No invoices found for the user" });
    }
...
router.post("/", async (req, res) => {
  try {
    ...
    // add new invoice to  'invoices' of User
  ...
  try {
    ...
    if (!invoice) {
      return res.status(404).json({
        success: false,
        message: "Invoice not found for the given entry item.",
      });
    }
    const updatedEntryItems = invoice.entryItem.filter(
      (item) => item.toString() !== entryItemId
    );
  ...
  Invoice.findByIdAndRemove(req.params.id)
    .then((user) => {
      if (user) {
        return res
          .status(200)
          .json({ success: true, message: "The invoice is deleted." });
      } else {
        return res
          .status(404)
